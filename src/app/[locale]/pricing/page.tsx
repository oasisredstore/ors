...
import { Check, Star, Loader2, CheckCircle } from "lucide-react";
import { activateSubscriptionAction } from "@/actions/subscription.actions";
...
export default function PricingPage() {
  ...
  const handleSubscribe = async (planId: string) => {
    ...
    if (result.error) {
      ...
      toast.success(
        isAr
          ? `✓ تم الاشتراك في ${plans.find(p => p.id === planId)?.[isAr ? "name" : "nameEn"] ?? planId}`
          : `✓ Subscribed to ${plans.find(p => p.id === planId)?.nameEn ?? planId}`
      );
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid md:grid-cols-3 gap-8 items-start">
          {plans.map((plan) => (
            <div
              key={plan.id}
              className={`relative bg-white rounded-3xl p-8 shadow-xl border-2 transition-all hover:-translate-y-2 duration-300 ${plan.color}`}
            >
              {plan.popular && (
              ...
              <div className="text-center mb-8">
                ...
                <div className="flex items-baseline justify-center gap-1">
                  <span className="text-4xl font-extrabold text-clay-900">{plan.price}</span>
              ...
              <button
                ...
                disabled={loading !== null || activatedPlan === plan.id}
                className={`w-full py-4 rounded-2xl text-white font-bold text-lg flex items-center justify-center gap-2 transition-all disabled:opacity-60 ${
                  activatedPlan === plan.id
                    ? "bg-green-600"
                    : plan.popular
                    ? "bg-oasis-600 hover:bg-oasis-700"
        ...
        <p className="text-center text-sm text-clay-400 mt-10">
          {isAr
            ? "* تُدفع الرسوم شهرياً. يمكنك الإلغاء في أي وقت."
            : "* Billed monthly. Cancel anytime. All prices in Algerian Dinar (DZD)."}
        </p>
      </div>
    </div>
  );
