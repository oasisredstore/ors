...
export default async function DashboardPage({ params }: DashboardPageProps) {
  ...
  const stats = [
    ...
    },
    {
      ...
      icon: Eye,
      color: "bg-oasis-100 text-oasis-700",
    },
    {
        ...
        {!artisan.isApproved && (
          <div className="mt-3 bg-amber-50 border border-amber-200 rounded-xl p-4">
            <p className="text-sm text-amber-800 font-medium">
              {isAr
                ? "⏳ حسابك في انتظار موافقة المشرف. سيتم إعلامك عند القبول."
      ...
      <div className="bg-white rounded-2xl border border-desert-200 p-6">
        <div className="flex items-center justify-between mb-5">
          ...
          <Link
            ...
            className="text-sm text-sand-600 hover:text-sand-700 font-medium"
          >
              ...
              return (
                <div key={p.id} className="flex items-center gap-3 py-2 border-b border-desert-100 last:border-0">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {avgRating && (
                      <p className="text-xs text-amber-500">★ {avgRating.toFixed(1)}</p>
                    )}
                  </div>
        ...
        <Link
          ...
          className="mt-4 block text-center bg-gradient-to-r from-sand-500 to-sand-600 text-white text-sm font-semibold py-2.5 rounded-xl hover:from-sand-600 hover:to-sand-700 transition-all"
        >
